...
export default function AdminDashboard() {
  ...
  const loadMainStats = async (token: string) => {
    try {
      ...
      setError('Gagal memuat data utama admin.');
    }
  };
  ...
  const handleSendAdminChatMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedChatUserId || !newChatMessage.trim()) return;
    const token = localStorage.getItem('token');
    if (!token) return;

    const textToSend = newChatMessage;
    setNewChatMessage('');
    ...
    try {
      ...
      if (res.ok) {
        await fetchChatMessages(selectedChatUserId);
  ...
  // Realtime Polling for Chat Conversations
  useEffect(() => {
    if (activeTab === 'chats') {
      fetchConversations();
  ...
  // Realtime Polling for Selected Chat Messages
  useEffect(() => {
    if (activeTab === 'chats' && selectedChatUserId !== null) {
      fetchChatMessages(selectedChatUserId);
      
      const channel = supabase
        .channel(`admin-messages-${selectedChatUserId}`)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'chats' }, () => {
          fetchChatMessages(selectedChatUserId);
        })
        .subscribe();
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .logo { font-size: 24px; font-weight: 900; color: #00bcd4; text-transform: uppercase; }
        ...
        <body>
          <div class="header">
            <div>
              ...
              <div style="font-size: 14px; color: #666;">Predator Gym Portal (Admin)</div>
            </div>
  ...
  return (
    <div className="bg-[#0B0F19] min-h-screen text-white flex flex-col md:flex-row relative overflow-x-hidden">
      ...
      <div className="md:hidden bg-[#111625] border-b border-gray-800 p-4 flex justify-between items-center sticky top-0 z-40">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[8px] text-gym-primary font-bold tracking-widest uppercase">Admin Portal</span>
          </div>
        </div>
        <button
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mb-10 pt-2">
            ...
            <div>
              ...
              <span className="text-[10px] text-gym-primary font-bold tracking-widest uppercase">Admin Portal</span>
            </div>
          </div>
          ...
          <nav className="space-y-1">
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-bold cursor-pointer ${activeTab === 'dashboard'
                  ? 'bg-gym-primary text-black shadow-[0_4px_20px_rgba(255,144,0,0.25)]'
            ...
            <button
              ...
              className={`w-full flex items-center justify-between px-4 py-3.5 rounded-xl text-sm font-bold cursor-pointer ${activeTab === 'chats'
                  ? 'bg-gym-primary text-black shadow-[0_4px_20px_rgba(255,144,0,0.25)]'
                  : 'text-gray-400 hover:bg-gray-800/40 hover:text-white'
                }`}
            >
              ...
              {conversations.reduce((sum, c) => sum + (c.unread_count || 0), 0) > 0 && (
                <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${activeTab === 'chats' ? 'bg-black text-gym-primary' : 'bg-red-500 text-white'
                  }`}>
      ...
      <main className="flex-grow p-4 sm:p-8 md:p-10 overflow-y-auto h-[calc(100vh-72px)] md:h-screen">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 bg-[#1e1e24]/60 backdrop-blur-md p-6 sm:p-8 rounded-3xl border border-gray-800 relative overflow-hidden">
          ...
          <div>
            <div className="flex items-center gap-2">
              <Shield size={16} className="text-gym-primary" />
              <span className="text-gym-primary font-bold tracking-widest text-[10px] uppercase">
                ...
                {activeTab === 'chats' && 'Real-time Live Chat Inquiries'}
            ...
            <h1 className="text-2xl sm:text-3xl font-black uppercase tracking-wide mt-1">
              ...
              {activeTab === 'chats' && 'Live Chat Member'}
            ...
            <p className="text-gray-400 text-xs sm:text-sm mt-1.5">
              ...
              {activeTab === 'chats' && 'Balas chat dan keluhan member Predator Gym secara real-time.'}
              {activeTab === 'walk-ins' && 'Catat tamu harian non-member dan pantau total omzet masuk dari tiket masuk.'}
            </p>
          </div>
          <div className="bg-gym-primary/10 border border-gym-primary/30 rounded-2xl px-4 py-3 flex items-center gap-3">
            <Sparkles className="text-gym-primary" size={18} />
            <div className="text-right">
              <span className="text-[10px] text-gray-400 block font-bold">System Status</span>
              <span className="text-xs font-black text-gym-primary uppercase">Sandbox Online</span>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
                <div className="bg-[#1e1e24] p-6 rounded-2xl border border-gray-800 shadow-md">
                  <div className="flex justify-between items-start">
                    ...
                    <Users className="text-gym-primary" size={20} />
                  </div>
                ...
                <div className="bg-[#1e1e24] p-6 rounded-2xl border border-gray-800 shadow-md">
                  <div className="flex justify-between items-start">
                    ...
                    <Award className="text-gym-primary" size={20} />
                  </div>
            ...
            <div className="bg-[#1e1e24] p-6 sm:p-8 rounded-3xl border border-gray-800 shadow-xl">
              <h2 className="text-xl font-extrabold text-white mb-6 tracking-wide border-l-4 border-gym-primary pl-3 flex items-center gap-2">
                <CalendarDays size={20} className="text-gym-primary" />
          ...
          <div className="bg-[#1e1e24] p-6 sm:p-8 rounded-3xl border border-gray-800 shadow-xl space-y-6">
            ...
            <div className="bg-[#303038]/20 p-5 rounded-2xl border border-gray-800/60 space-y-4">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <h2 className="text-lg font-extrabold text-white tracking-wide border-l-4 border-gym-primary pl-3 flex items-center gap-2">
                  <Receipt size={18} className="text-gym-primary" />
                ...
                {(transSearch || startDate || endDate) && (
                  <button
                    ...
                    className="text-xs text-gym-primary hover:underline font-bold flex items-center gap-1.5 cursor-pointer ml-auto lg:ml-0"
                  >
                    <X size={14} />
                    Bersihkan Filter
                  </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-[#303038]/50 border border-gray-800 focus:border-gym-primary text-white text-xs rounded-xl pl-11 pr-4 py-3 outline-none"
                  />
                </div>

                {/* Start Date */}
                <div className="flex items-center gap-2 bg-[#303038]/50 border border-gray-800 rounded-xl px-4 py-1.5 focus-within:border-gym-primary">
            ...
            <div className="flex gap-2 border-b border-gray-800 pb-2">
              <button
                ...
                className={`px-4 py-2 text-sm font-bold rounded-lg transition-all ${transTab === 'membership' ? 'bg-gym-primary text-black' : 'text-gray-400 hover:bg-gray-800/40 hover:text-white'}`}
              >
                Membership
              </button>
              <button
                onClick={() => setTransTab('pt')}
                className={`px-4 py-2 text-sm font-bold rounded-lg transition-all ${transTab === 'pt' ? 'bg-gym-primary text-black' : 'text-gray-400 hover:bg-gray-800/40 hover:text-white'}`}
              >
                Personal Trainer
              </button>
            </div>
                  ...
                  {filteredSubscriptions.length > 0 ? (
                    filteredSubscriptions.map(s => (
                      <tr key={s.id} className="hover:bg-gray-800/20">
                        ...
                        <td className="py-4 px-4 font-mono text-xs text-gym-primary font-bold">
                          {s.order_id || `SUB-${s.id}`}
                        </td>
                        ...
                        <td className="py-4 px-4 text-gray-300">
                          ...
                          {s.trainer && (
                            <div className="text-xs text-gym-primary mt-1 font-semibold flex items-center gap-1">
                              <span>👩‍🏫 PT:</span>
        ...
        {activeTab === 'members' && (
          <div className="bg-[#1e1e24] p-6 sm:p-8 rounded-3xl border border-gray-800 shadow-xl space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <h2 className="text-xl font-extrabold text-white tracking-wide border-l-4 border-gym-primary pl-3 flex items-center gap-2">
                <Users size={20} className="text-gym-primary" />
              ...
              <div className="relative w-full sm:w-80">
                ...
                <input
                  ...
                  className="w-full bg-[#303038]/60 border border-gray-800 focus:border-gym-primary text-white text-sm rounded-xl pl-12 pr-4 py-3 outline-none"
                />
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-gray-800 text-gray-400 text-xs font-bold uppercase tracking-wider">
                    ...
                    <th className="py-3 px-4 text-center">Kuota Trainer</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800/40 text-sm">
                      ...
                      return (
                        <tr key={m.id} className="hover:bg-gray-800/20">
                          ...
                          <td className="py-4 px-4 text-center font-bold text-white">
                            {m.trainer_subscription_end_date ? new Date(m.trainer_subscription_end_date).toLocaleDateString('id-ID') : '-'}
                          </td>
                        </tr>
                      );
                    })
          ...
          <div className="grid grid-cols-1 gap-8 items-start">
            ...
            <div className="bg-[#1e1e24] p-6 sm:p-8 rounded-3xl border border-gray-800 shadow-xl space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-800 pb-5">
                <h3 className="text-lg font-bold text-white uppercase tracking-wider border-l-4 border-gym-primary pl-3 flex items-center gap-2">
                  <Briefcase size={18} className="text-gym-primary" />
        ...
        {activeTab === 'financials' && (
          <div className="space-y-8">
            ...
            <div className="bg-[#1e1e24] p-6 rounded-3xl border border-gray-800 shadow-xl space-y-4">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <h3 className="text-lg font-bold text-white uppercase tracking-wider border-l-4 border-gym-primary pl-3 flex items-center gap-2">
                  <TrendingUp size={18} className="text-gym-primary" />
                ...
                <div className="flex flex-wrap items-center gap-3">
                  {(startDate || endDate || finTab !== 'all') && (
                    <button
                      ...
                      className="text-xs text-gym-primary hover:underline font-bold flex items-center gap-1.5 cursor-pointer"
                    >
                      <X size={14} />
                      Bersihkan Filter
                    </button>
                      ...
                      filteredJournalList.map(item => (
                        <tr key={item.id} className="hover:bg-gray-800/20">
                          ...
                          <td className="py-4 px-4 text-gray-400 flex items-center gap-2">
                            <CreditCard size={14} className={item.type === 'walk-in' ? 'text-green-400' : 'text-gym-primary'} />
                            {item.method}
                          </td>
        ...
        {activeTab === 'gym-features' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-4 bg-[#1e1e24] p-6 sm:p-8 rounded-3xl border border-gray-800 shadow-xl space-y-6">
              <h3 className="text-lg font-bold text-white uppercase tracking-wider border-l-4 border-gym-primary pl-3">
              ...
              <form onSubmit={handleSaveFeature} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#303038]/50 border border-gray-800 focus:border-gym-primary text-white text-xs md:text-sm rounded-xl px-4 py-3.5 outline-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-[#303038]/50 border border-gray-800 focus:border-gym-primary text-white text-xs md:text-sm rounded-xl px-4 py-3.5 outline-none cursor-pointer"
                  >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-[#303038]/50 border border-gray-800 focus:border-gym-primary text-white text-xs md:text-sm rounded-xl px-4 py-3.5 outline-none resize-none"
                  />
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="flex-grow btn-primary py-3.5 text-xs font-extrabold uppercase tracking-widest bg-gradient-to-r from-orange-500 to-orange-600 border-transparent shadow-none hover:scale-100 rounded-xl cursor-pointer"
                  >
                  ...
                  <tbody className="divide-y divide-gray-800/40 text-sm">
                    {filteredGymFeatures.length > 0 ? (
                      filteredGymFeatures.map((item) => {
                        return (
                          <tr key={item.id} className="hover:bg-gray-800/20">
                            <td className="py-4 px-4 text-center text-gym-primary">
        ...
        {activeTab === 'chats' && (
          <div className="bg-[#1e1e24] rounded-3xl border border-gray-800 shadow-xl overflow-hidden h-[600px] flex">
            {/* Conversations List (Left Side) */}
            <div className="w-80 border-r border-gray-800 flex flex-col h-full bg-[#111625]/40 shrink-0">
              <div className="p-4 border-b border-gray-800">
                <h3 className="font-extrabold text-xs uppercase tracking-wider text-white">Percakapan Member</h3>
              </div>
              <div className="flex-1 overflow-y-auto divide-y divide-gray-800/30">
                {conversations.length === 0 ? (
                  <div className="p-6 text-center text-gray-500 text-xs">Belum ada chat masuk dari member.</div>
                ) : (
                  conversations.map((conv) => {
                    const isSelected = selectedChatUserId === conv.user.id;
                    return (
                      <button
                        key={conv.user.id}
                        onClick={() => setSelectedChatUserId(conv.user.id)}
                        className={`w-full text-left p-4 flex justify-between items-start gap-2 cursor-pointer ${isSelected ? 'bg-gym-primary/10 border-l-4 border-gym-primary' : 'hover:bg-gray-800/30'
                          }`}
                      >
            ...
            {/* Conversation Window (Right Side) */}
            <div className="flex-1 flex flex-col h-full bg-[#0B0F19]/40">
              {selectedChatUserId === null ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center p-6 space-y-3">
                  <div className="w-16 h-16 rounded-full bg-gym-primary/10 flex items-center justify-center text-gym-primary">
                    <MessageSquare size={32} />
                  </div>
                  <h4 className="font-extrabold text-white">Live Chat CS Predator Gym</h4>
                  <p className="text-sm text-gray-400 max-w-sm">Pilih salah satu percakapan di sebelah kiri untuk melihat pesan dan mulai merespon keluhan member.</p>
                </div>
              ) : (
                <>
                  {/* Chat Header */}
                  {(() => {
                    const activeConv = conversations.find(c => c.user.id === selectedChatUserId);
                    if (!activeConv) return null;
                    return (
                      <div className="p-4 border-b border-gray-800 bg-[#1e1e24] flex justify-between items-center">
                        <div>
                          <div className="flex items-center gap-3">
                            <h3 className="font-bold text-white text-sm">{activeConv.user.name}</h3>
                            {lastSyncTime && (
                              <span className="text-[9px] text-gray-500 bg-gray-800 px-2 py-0.5 rounded-md">Sync: {lastSyncTime}</span>
                            )}
                          </div>
                          <p className="text-xs text-gray-400 mt-0.5">Email: {activeConv.user.email} | WA: {activeConv.user.whatsapp || '-'}</p>
                        </div>
                      </div>
                    );
                  })()}
                  ...
                  <div className="flex-grow overflow-y-auto p-4 space-y-3">
                    {chatMessages.map((msg, index) => {
                      const isMe = msg.sender_id !== selectedChatUserId;
                      return (
                  ...
                  {/* Input Form */}
                  <form onSubmit={handleSendAdminChatMessage} className="p-4 bg-[#1e1e24] border-t border-gray-800 flex gap-2">
                    <input
                      type="text"
                      value={newChatMessage}
                      onChange={(e) => setNewChatMessage(e.target.value)}
                      placeholder="Ketik balasan Anda..."
                      className="flex-grow bg-gray-900 border border-gray-800 focus:border-gym-primary text-white text-sm rounded-xl px-4 py-3 outline-none"
                    />
                    <button
                      type="submit"
                      className="px-5 bg-gym-primary hover:bg-orange-600 text-black font-extrabold text-xs uppercase tracking-wider rounded-xl cursor-pointer flex items-center justify-center gap-1.5 shrink-0"
                    >
                      <Send size={14} />
                      Kirim
                    </button>
              ...
              <form onSubmit={handleWalkInSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#303038]/50 border border-gray-800 focus:border-gym-primary text-white text-xs md:text-sm rounded-xl px-4 py-3.5 outline-none [color-scheme:dark]"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full bg-gym-primary hover:bg-orange-600 text-black py-3.5 rounded-xl text-xs font-extrabold uppercase tracking-wider disabled:opacity-50 transition-all cursor-pointer font-bold"
                >
